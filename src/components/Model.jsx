            ...
            <div className="flex-center">
              <ul className="color-container">
                {models.map((item, i) => (
                  <li 
                    key={i} 
                    className="w-6 h-6 rounded-full mx-2 cursor-pointer relative" 
                    style={{ backgroundColor: item.color[0] }} 
                    onClick={() => setModel(item)} 
                  >
                    {model.id === item.id && (
                      <span className="absolute -inset-1 border-2 border-white rounded-full pointer-events-none"></span>
                    )}
                  </li>
                ))}
              </ul>
